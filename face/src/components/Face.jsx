...
import { Upload, Play, Share2, Eye, Users, Video, LogOut, Plus, Trash2, Edit, ExternalLink, Clock, MapPin, Globe, UserCheck, AlertCircle } from 'lucide-react';
...
const VideoPlatformApp = () => {
  ...
  const fetchImages = async () => {
    try {
      ...
      if (response.ok) {
        ...
        // Add mock data with views and duration if no real data
        const mockData = data.length === 0 ? [
          {
            id: 1,
            title: "Amazing Nature Documentary",
            description: "Explore the wonders of wildlife in this breathtaking documentary featuring stunning landscapes and rare animal behaviors.",
          ...
          ...item,
          views: item.views || 1250,
          duration: item.duration || "6:30"
        }));
        setImages(mockData);
      } else {
  ...
  const getImageUrl = (imagePath) => {
    ...
    // If it's just a filename/path without leading slash
  ...
  const generateShareableLink = async (image) => {
    try {
      ...
      const imageParams = new URLSearchParams({
        ...
        type: 'video',
        image_url: getImageUrl(image.image),
                      ...
                      <div key={image.id} className="bg-white rounded-lg shadow-lg overflow-hidden border border-gray-200">
                        ...
                        <div className="relative">
                          <img 
                            ...
                            onError={(e) => {
                              console.error('Content failed to load:', image.image);
                              e.target.style.display = 'none';
                              const errorDiv = document.createElement('div');
                              errorDiv.className = 'w-full h-48 bg-gray-200 flex items-center justify-center text-gray-500';
                              errorDiv.innerHTML = '<div class="text-center"><p>Content failed to load</p></div>';
